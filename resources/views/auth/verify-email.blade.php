...
<div class="col-md-6" style="background-color:  #313131; height:100vh;">
    <div class="sign-in-from text-center">
        ...
        <div class="card">
            ...
            @endif
            <form class="mt-5" method="POST" action="{{ route('password.verify') }}">
                @csrf
                <div class=" form-group text-start" ml-3>
                    <h6 class="form-label fw-bold" style="margin-left:10px">Otp</h6>
                    <input type="number" class="form-control mb-0 {{ $errors->has('otp') ? 'is-invalid' : '' }}"
                        placeholder="Enter Your Otp" required value="{{ old('otp') }}" name="otp">
                    @if ($errors->has('otp'))
                    <span class="text-danger">{{ $errors->first('otp') }}</span>
                    @endif
                </div>
                <button type="submit" class="btn btn-primary mt-4 fw-semibold text-uppercase w-100">Verify Otp</button>
            </form>
            <h6 class="mt-5 m-5">Didn't get the code? <a href="{{ route('resend.otp') }}">Resend OTP</a> OR <a href="{{ route('register') }}"> sign up</a></h6>
        </div>
    </div>
</div>
@endsection